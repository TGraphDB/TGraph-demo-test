...
datasetSize_array=("1day" "7day")
db_hour_array=("1h" "12h")
query_start_array=(201005011900 201005010800 201005071900 201005070800)
query_end_array=(201005012000 201005012000 201005072000 201005072000)


day_length=${#datasetSize_array[@]}
hour_length=${#db_hour_array[@]}

function serverAutotest() {

  for (( i = 0; i < day_length; i++ )); do
    export DB_PATH="C:\tgraph\test-db\Tgraph-bj60c-${datasetSize_array[i]}"

    echo -e "\033[47;30m [Tgraph Test Info]---------------[ DataBase Information : Tgraph-bj60c-${datasetSize_array[i]} ]--------------- \033[0m"
...
function clientAutotest() {

  export MAX_CONNECTION_CNT=1

  for (( i = 0; i < day_length; i++ )); do

    for (( j = 0; j < hour_length; j++ )); do
      fileName="Tgraph-${datasetSize_array[i]}-${db_hour_array[j]}-result"
      RESULT_DATA_PATH="E:\tgraph\test-result"\\"$fileName"
      if [ ! -d "$RESULT_DATA_PATH" ]; then
          mkdir $RESULT_DATA_PATH
      fi
      export RESULT_DATA_PATH
      TEMPORAL_DATA_START=${query_start_array[2*i+j]}
      TEMPORAL_DATA_END=${query_end_array[2*i+j]}
      export TEMPORAL_DATA_START
      export TEMPORAL_DATA_END

      echo -e "\033[47;30m [Tgraph Test Info]---------------[ Test Information : Tgraph-60c-${datasetSize_array[i]}-${db_hour_array[j]} ]--------------- \033[0m"
      echo -e "\033[47;30m [Tgraph Test Info]---------------[ Start Time : $TEMPORAL_DATA_START ]--------------- \033[0m"
      echo -e "\033[47;30m [Tgraph Test Info]---------------[ End Time : $TEMPORAL_DATA_END ]--------------- \033[0m"
      echo -e "\033[47;30m [Tgraph Test Info]---------------[ Result File : $fileName ]--------------- \033[0m"

      echo -e "\033[47;30m [Tgraph Test Info]---------------[ Tests are about to start ]--------------- \033[0m"
      sleep 5
      LOG_TEST_NAME="TGraph(SnapshotTest-${datasetSize_array[i]}-${db_hour_array[j]})"
      ...
      LOG_TEST_NAME="TGraph(SnapshotAggregationMaxTest-${datasetSize_array[i]}-${db_hour_array[j]})"
      export LOG_TEST_NAME
      echo -e "\033[47;30m [Tgraph Test Info]---------------[ $LOG_TEST_NAME is about to start ]---------------$(date "+%Y-%m-%d %H:%M:%S") \033[0m"
      sleep 5
      ...
      LOG_TEST_NAME="TGraph(SnapshotAggregationDurationTest-${datasetSize_array[i]}-${db_hour_array[j]})"
      export LOG_TEST_NAME
      echo -e "\033[47;30m [Tgraph Test Info]---------------[ $LOG_TEST_NAME is about to start ]---------------$(date "+%Y-%m-%d %H:%M:%S") \033[0m"
      sleep 5
      ...
      LOG_TEST_NAME="TGraph(EntityTemporalConditionTest-${datasetSize_array[i]}-${db_hour_array[j]})"
      export LOG_TEST_NAME
      echo -e "\033[47;30m [Tgraph Test Info]---------------[ $LOG_TEST_NAME is about to start ]---------------$(date "+%Y-%m-%d %H:%M:%S") \033[0m"
      sleep 5
...
#      LOG_TEST_NAME="TGraph(ReachableAreaQueryTest-${datasetSize_array[i]}-${db_hour_array[j]})"
#      export LOG_TEST_NAME
#      echo -e "\033[47;30m [Tgraph Test Info]---------------[ $LOG_TEST_NAME is about to start ]---------------$(date "+%Y-%m-%d %H:%M:%S") \033[0m"
#      sleep 5
#      runReachableAreaQueryTest
#      echo -e "\033[47;30m [Tgraph Test Info]---------------[ End of ReachableAreaQueryTest ]--------------- \033[0m"
...
function clientAutoTestWithIndex() {
  ...
  INDEX_ID_OF_MAX=$firstline
  export INDEX_ID_OF_MAX
  cd $SHELL_FILE_PATH
  ...
  INDEX_ID_OF_DURATION=$firstline
  export INDEX_ID_OF_DURATION
  cd $SHELL_FILE_PATH
